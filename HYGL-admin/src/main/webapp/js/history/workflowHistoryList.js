            ...
            $('#ProcessHistory-table').bootstrapTable({
                ...
                columns : [{
	             ...
	             },
	             {
                    ...
                    formatter: function (value, row, index) {
                        var retStr = "";
                         retStr= '<a href="javascript:void(0);" onclick="lookJobHistoryList(\''+row.process_id+'\',\''+row.version+'\',\''+row.status+'\');" ><span style="color:blue;">查看</span></a>';
                        return retStr;
                    }
                }],
...
function lookJobHistoryList(processId,version,status){
	//先关闭作业运行历史和作业详情
	closeTab(666);
	closeTab(665);
	$(".active").removeClass("active");
	if ($("#content666").length){//如果当前tab已存在
		clickTab(666,"history/toJobPage?id="+processId+"&version="+version+"&runStatus="+status);
		$("#tab_666").addClass('active');
		$("#content666").addClass("active").addClass("in");;//iframe内容显示隐藏
		
	}else{
		 $('#myTab').append("<li id='tab_666'><a onclick=clickTab(666,'history/toJobPage?id="+processId+"&version="+version+"&runStatus="+status+"') href='#content666' data-toggle='pill'>作业运行历史<i onclick='closeTab(666)' class='fa fa-remove tab-close' style='padding-left: 3px;'></i></a></li>");

  ...
  //初始化流程类型筛选条件
    ...
    $.ajax({
    	...
    	success:function(data){
    		...
    		$("#detail_search_job_type").append(h);//append 添加进去并展示  
    		$("#detail_search_job_type").on(  
    				"change"
    		)
    	},
    });
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $("#search_job_type_first").append(h);//append 添加进去并展示  
            $("#search_job_type_first").on(  
	            "change"
	           )
        },
    });
...
var Job = function () {
    return{
        ...
        var temp = {
            ...
            jobType:$("#search_job_type_first").val(),
        };
        return temp;
        },
        searchJob:function () {
     ...
     download:function (){
      ...
      openUpdateModal:function () {
        ...
        deleteJob:function () {
        	...
        	var getSelectRows = $("#job-table").bootstrapTable('getSelections', function (row) {
  	          return row;
	  		});
        ...
        openJobDetailModel:function () {
        	if(this.checkSingleData()) {
        		...
        		$("#job--log-table").bootstrapTable('destroy');
        		$('#job--log-table').bootstrapTable({
                      url: "processJobUpdateHistory/getHistoryList?jobBasicId="+id,
                      method:"post",
                      dataType: "json",
                      contentType: "application/x-www-form-urlencoded",
                      striped:true,//隔行变色
        ...
        closeDlg:function () {
            ...
            $("#jobDetailMydlg").modal('hide');
		    document.getElementById("saveJobButton").removeAttribute("disabled");
            Job.formValidator();
        },//上传作业
       importRarJob:function(flag) {
           var formData = new FormData();
           var name = $("#rarfileName").val();
           if("2"==flag){
        	   name = $("#update_rarfileName").val();
           }
           if(!/[.*\.rar|.kjb]$/.test(name)){
        	   $.alert({
                   title: '提示信息！',
                   content: '只能上传rar或者kjb文件！',
                   type: 'red'
               }); 
        	   return ;
           }
           var files = $("#rarfileName")[0].files[0];
           if("2"==flag){
        	   files = $("#update_rarfileName")[0].files[0];
            }
        	formData.append("file", files);
        	formData.append("name", name);
           $.ajax({
               url: "jobBasic/uploadRar",
               type: 'POST',
               data: formData,
               ...
               success: function (responseStr) {
            	   ...
            	   if(responseStr.result){
            		   ...
            		   if("2"==flag){ 
	            		   $("#job_update_jobUrl").val(responseStr.message);
	            		   $("#job_update_jobUrl").attr("readonly", true);
            		   }else{
            			   $("#jobUrl").val(responseStr.message);
	            		   $("#jobUrl").attr("readonly", true);
            		   }
            	   }else{
            		   $.alert({
                           title: '提示信息！',
                           content: '上传失败！' + responseStr.message,
                           type: 'blue'
                       });
            	   }
               }
               ,
               error : function (打他onseStr) {
            	   $.alert({
                       title: '提示信息！',
                       content: '上传失败！',
                       type: 'blue'

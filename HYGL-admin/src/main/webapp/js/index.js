...
var tabScroll = {
	    rollPage: function(direction, i) {
	        var tabul = $("#myTab"),
	            tabli = tabul.children("li"),
	            tabulW = (tabul.prop("scrollWidth"), tabul.outerWidth()),
	            tabulLeft = parseFloat(tabul.css("left"));
	        if ("left" === direction) {//向左
	            if (!tabulLeft && tabulLeft <= 0)
	                return;
	            var r = -tabulLeft - tabulW;
	            tabli.each(function(e, i) {
	                var n = $(i),
	                l = n.position().left;
	                if (l >= r) return tabul.css("left", -l),!1
	            })
	        } else "auto" === direction ? !function() {
	            var e,
	                r = tabli.eq(i);
	            if (r[0]) {
	                if (e = r.position().left, e < -tabulLeft)
	                    return tabul.css("left", -e);
	                if (e + r.outerWidth() >= tabulW - tabulLeft) {
	                    var o = e + r.outerWidth() - (tabulW - tabulLeft);
	                    tabli.each(function(e, i) {
	                        var n = $(i),
	                        l = n.position().left;
	                        if (l + tabulLeft > 0 && l - tabulLeft > o)
	                            return tabul.css("left", -l),!1
	                    })
	                }
	            }
	        } () : tabli.each(function(e, i) {
	            var n = $(i),
	            r = n.position().left;
	            if (r + n.outerWidth() >= tabulW - tabulLeft)
	                return tabul.css("left", -r),!1
	        })
	    },
	    leftPage: function() {

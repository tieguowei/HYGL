...
function closeTab (id) {
	//如果关闭的是流程监控和流程运行历史，子节点都关闭
	var history = $("#menu_history").val();
	var monitor = $("#menu_monitor").val();
	
	if(id == history){
		if ($("#content666").length){
			closeTab(666);
		}
		if ($("#content665").length){
			closeTab(665);
		}
	}
	
	if(id == monitor){
		if ($("#content555").length){
			closeTab(555);
		}
		if ($("#content554").length){
			closeTab(554);
		}
	}
 ...
 * 解决closeTab冲突问题
 */
function closeAllSon(){
	if ($("#content999").length){//如果当前tab已存在 关闭
		closeTab(999);
	}
	if ($("#content888").length){
		closeTab(888);
	}
	if ($("#content777").length){
		closeTab(777);
	}
	if ($("#content666").length){
		closeTab(666);
